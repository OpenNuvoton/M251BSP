            ...
            switch (g_sCBW.u8OPCode)
            {
                ...
                case UFI_READ_10:
                {
                    ...
                    if (g_u32Length == 0)
                    {
                        ...
                        /*
                            FIXME:For passing the USB-IF MSC Test, Command Set Test-Device Configured:
                            Some test items in this test will try to read data(IN) with the size more than the device.
                            It will make the device hardfault. Use the following implementation to solved.
                        */
                        if ((Hcount > DATA_FLASH_STORAGE_SIZE))
                        {
                            g_u8Prevent = 1;
                            g_u32CbwStall = 1;
                            USBD_SET_EP_STALL(EP2);
                ...
                case UFI_WRITE_10:
                {
                    ...
                    /*
                        For passing the USB-IF MSC Test, Command Set Test-Device Configured:
                        Some test items in this test will try to send data(OUT) with the size more than the device.
                        It will make the device hardfault. Use the following implementation to solved.
                    */
                    if ((Hcount > DATA_FLASH_STORAGE_SIZE))
                    {
